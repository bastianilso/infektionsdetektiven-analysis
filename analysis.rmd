...
test%<>%
  ...
  #select(PlayID, Event, Timestamp, ViewStatsTimestamp)%>%
    mutate(
...
ggplot(data.frame(test[test$prev_view_time<250,]), aes(x=as.numeric(prev_view_time), y=GameTime)) + geom_jitter(width=0.1,size = 1,alpha=.2)+theme_bw()
+ facet_grid(rows = as.factor(LevelNo))
