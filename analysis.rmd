...
### Statistics time viewed, all grades
...
test<-df_experiment %>% inner_join(D_vt)
...
#ggplot(test[test$prev_view_time<250,], aes_(x=~view_time, y=~max_level_reached)) + geom_jitter(width=.1, size = 1, alpha=.2)

test %>% filter(Event == 'NextLevel') %>%
ggplot(test, mapping= aes_(x=as.numeric(view_time), y=as.numeric(max_level_reached))) + geom_jitter(width=.1, size=1, alpha=.2) + facet_grid(.  ~LevelNo)
