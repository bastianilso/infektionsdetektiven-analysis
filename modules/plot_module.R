...
plot_module_summary <- function(input, output, session, df) {
  ...
  vistemplate <- plot_ly() %>%
    ...
    layout(dragmode = "pan", showlegend=FALSE,
           xaxis=list(mirror=T, showline=F,titlefont = list(size=16),tickfont = list(size=14), range=c(0,50)),
  ...
  output$infection_spread <- renderPlotly({
    ...
    subplot(nrows = 2, titleX = TRUE, titleY = TRUE, margin = 0.08, fig1, fig2, fig3, fig4, fig5, fig6, fig7) %>%
      layout(height = 700, widths=c(1,1,1,1,1,1,1))
